...
df |> 
  ggplot(mapping = aes(x = x, y = y)) +
  # geoms
  geom_segment(mapping = aes(xend = x, yend = predicted),
            alpha = .3, linetype = "dashed") +
  geom_point(color = pal[6], size = 4, alpha = .7) +
  geom_line(aes(y = predicted), color = pal[3], linewidth = 1) +
  # geom_smooth(method = "lm", se = F, color = pal[3]) +
  # text annotations
  ...
  # theme
  scale_x_continuous(limits = c(0, max(df$x) + 1)) +
