...
![DAG Directo](images/dag_directo.PNG)

![DAG confusión](images/dag_confusion.PNG)
...
En la ecuación el valor de `y` es 2,45 ($\beta_0$) + 0,78\* el valor de `x` ($\beta_1*x$).
...
El coeficiente $\beta_1$ de la variable independiente principal `x` varió al incorporar la nueva variable `z`, pasando de 0,78 (cruda) a 0,65 (ajustada), es decir que disminuyó casi un 20%. A la vez, la covariable tiene una relación significativa con la variable dependiente `y`.

Entonces podemos ver que la regresión ajustó el efecto de `x` sobre `y`, teniendo en cuenta el efecto confusor de `z`.

El valor de `y` es 0,66 ($\beta_0$) + 0,65\* el valor de `x` ($\beta_1*x$) mientras `z` es igual al nivel de referencia **A**, en cambio `y` vale 0,66 ($\beta_0$) + 0,65\* el valor de `x` ($\beta_1*x$)  + 4,55 ($\beta_2$) cuando `z` es igual a **B**.
...
Para quienes necesiten repasar el tema, pueden leer el documento de @de2001que del siguiente [\[Link\]](https://halweb.uc3m.es/esp/personal/personas/amalonso/esp/bstat-tema8vme.pdf){.uri}
...
El valor de `y` es 2,77 ($\beta_0$) + 0,23\* el valor de `x` ($\beta_1*x$) mientras `z` es igual al nivel de referencia **A**, en cambio `y` vale 2,77 ($\beta_0$) + 0,23\* el valor de `x` ($\beta_1*x$)  + 2,49 ($\beta_2$) + 0,46\*x ($\beta_3*x$) cuando `z` es igual a **B**. Observamos una sinergia entre el nivel **B** de la variable `z` y la variable `x` en el efecto causado a la variable `y`.
