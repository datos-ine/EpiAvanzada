...
Según @gordis2017, en un estudio sobre si la exposición A es una causa de la enfermedad B, se dice que un tercer factor, el factor X, es un factor de confusión si se cumple lo siguiente:

1.  El factor X es un factor de riesgo conocido para la enfermedad B.
2.  El factor X se asocia con la exposición A, pero no es un resultado de la exposición A.
...
Para quienes necesitan repasar este tema, pueden leer el artículo de @deirala2001 o descargarlo desde [aquí](http://halweb.uc3m.es/esp/Personal/personas/amalonso/esp/bstat-tema8vc.pdf).
...
Durante el curso vamos a utilizar la regresión lineal y otras regresiones lineales generalizadas para resolver la confusión ajustando los efectos de las variables.
...
datos_conf |> 
  ggplot(aes(x = x, y = y)) +
  geom_point(color = "#49917a", size = 2) +
  geom_smooth(method = "lm", se = F, color = "#1e6590")
```
...
El intercepto ($\beta_0$) es de 2,45 y el coeficiente $\beta_1$ (pendiente) significativo de 0,78.
...
El gráfico de dispersión muestra que hay una diferencia entre las rectas de regresión que se mantiene prácticamente constante (paralelas) en todo su desarrollo. Esa distancia medida en valores de `y` es $\beta_2$, para la ecuación:
...
El coeficiente $\beta_1$ de la variable independiente principal `x` varió al incorporar la nueva variable `z`, pasando de 0,78 (cruda) a 0,61 (ajustada), es decir que disminuyó más de un 20%. A la vez, la covariable tiene una relación significativa con la variable dependiente `y`.

Entonces podemos ver que la regresión ajustó el efecto de `x` sobre `y`, teniendo en cuenta el efecto confusor de `z`.
...
Para quienes necesiten repasar el tema, pueden leer el documento del siguiente [\[Link\]](https://halweb.uc3m.es/esp/personal/personas/amalonso/esp/bstat-tema8vme.pdf){.uri}
...
El término de interacción es significativo, aunque la variable `z` pareciese que no, es decir la significación se asocia a uno de los niveles (la categoría **B**). No hay manera de escindir un nivel del otro por lo que debemos dejar la variable `z`.

El valor de `y` es 2,77 ($\beta_0$) + 0,23\* el valor de `x` ($\beta_1*x$) mientras `z` es igual al nivel de referencia **A**, en cambio `y` vale 2,77 ($\beta_0$) + 0,23\* el valor de `x` ($\beta_1*x$)  + 2,49 ($\beta_2$) + 0,46\*x ($\beta_3*x$) cuando `z` es igual a **B**.
