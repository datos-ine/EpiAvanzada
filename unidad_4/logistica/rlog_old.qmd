...
ggplot(aes(x = x, y = y)) +
  ...
  labs(title = "Función de distribución", y = "F(x)") +
...
# carga datos
datos <- read_csv2("unidad_4/logistica/caminar.txt") |> 
...
`Pr(>|z|)` : *p*-valores para el test de Wald,
...
En forma similar al test $F$ parcial de la RLM, el test de Wald testea la $H_0 = \beta_i=0$. Hemos expresado el marco de prueba de hipótesis en términos de la pendiente $\beta_i$ porque esta forma se parece mucho a lo que estudiamos en regresión lineal. Sin embargo, los resultados de la regresión logística suelen expresarse en términos de OR.

Una pendiente de 0 es lo mismo que una razón de probabilidades (OR) de 1, por lo que a menudo expresamos la hipótesis nula de interés como *"la razón de probabilidades es 1"*. Esto significa que las dos probabilidades son iguales y la variable explicativa no es útil para predecirlas. Recordemos que en la regresión logística, los $\beta$ no se interpretan según los coeficientes observados en la salida del modelo, sino que debemos calcular su exponencial.

#### Máxima verosimilitud

Mientras que en la regresión lineal múltiple (RLM) los $\beta$ se obtenían por el método de los mínimos cuadrados, en el caso de la regresión logística, esto se obtienen por el método de **máxima verosimilitud** (ML por su nombre en inglés, *maximum likelihood*). El fundamento de esta técnica es utilizar la información disponible de los datos de la muestra para elegir el valor del parámetro para el cual es más probable haber observado ese resultado muestral. La ML entonces se calcula mediante un proceso iterativo.
...
Entonces, la calidad del ajuste de un modelo de regresión logística puede evaluarse mediante:

-   Análisis de la función desvío o *deviance* (también llamada devianza o lejanía).
-   Test de la *razón de verosimilitud* (LRT por su nombre en inglés *likelihood ratio test*)

#### Deviance

La *deviance* odevianza ($D$) se define como:
...
La devianza del modelo nulo es más grande que la de cualquier modelo ampliado. Esto es razonable, debido a que se trata de un modelo mucho menos sofisticado (que no incorpora información alguna de posibles variables “explicativas”) y debe necesariamente tener una incapacidad predictiva mayor.
...
El ***Likelihood Ratio Test**,* evalua la significancia de incorporar variables al modelo, comparándola con el modelo nulo. El estadístico tiene una distribución chi-cuadrado con grados de libertad equivalentes al número de predictores del modelo generado.

#### Criterio de Información de Akaike (AIC)

Es una medida de la calidad relativa de un modelo estadístico, para un conjunto dado de datos. Como tal, el AIC proporciona un medio para la selección del modelo. Lo definimos como:
...
Para comparar modelos de regresión logística, podemos usar: AIC, LRT o un ANOVA entre los modelos.

-   **AIC**: el primer término de la ecuación del AIC ($2k$) penaliza por la incorporación de variables al modelo, mientras que el segundo ($2ln(D)$) compensa por la bondad del ajuste. Dado un conjunto de modelos candidatos para los datos, el modelo preferido es el que tiene el valor mínimo de AIC.

-   ***Likelihood Ratio Test:*** también permite la comparación entre 2 modelos, no por el cociente entre el modelo nulo y el actual, sino entre la $V$ de dos modelos a comparar. El estadístico tiene una distribución Chi-cuadrado con grados de libertad dados por la diferencia de grados de libertad de los dos modelos comparados.

-   **ANOVA**: En R, la función `anova()` nos permite también la comparación entre modelos
...
De manera análoga al *Coeficiente de Determinación* ($R^2$) que se emplea para la Regresión Lineal, se han desarrollado varios coeficientes para estimar la proporción de varianza de la variable dependiente explicada por las variables independientes, aplicables los modelos de Regresión Logística. En R podemos calcular los siguientes coeficientes utilizando el paquete `performance`:
...
-   Coeficiente de **Nagelkarkle**

```{r}
r2_nagelkerke(mod1)
```

Este último es una versión corregida del anterior, cuyo valor máximo es menor a 1 incluso para un modelo perfecto, por eso, en muchos casos el $R^2$ de Cox y Snell tiende a subestimar el coeficiente.
...
Calcula el Coeficiente de Discriminación ($D$) para modelos lineales generalizados con variable respuesta binaria. Este es el coeficiente que usa por defecto `performance` cuando ingresamos un modelo de regresión logística, por lo que también se puede obtener como:

```{r}
r2(mod1)
```

Ninguno de estos coeficientes sirve para medir calidad de ajuste, para ello usaremos el **test de Hosmer-Lemeshow** que compara la probabilidad predicha por el modelo con la probabilidad de los datos de la muestra. El estadístico se calcula a través de varios grupos empleando los deciles de las probabilidades predichas por el modelo, y comparando las frecuencias observadas en dichos grupos con las esperadas. Este estadístico sigue una distribución de $\chi^2$ con $n-2$ grados de libertad ($n$= número de casos). En R podemos calcularlo con la función `performance_hosmer()` del paquete `performance`:

```{r}
performance_hosmer(mod1)
```

En un test de bondad de ajuste siempre en la hipótesis nula se afirma que el modelo propuesto se ajusta a lo observado en la realidad. Por lo tanto, un *p*-valor superior a 0.05 implica que lo que observamos se ajusta suficientemente a lo que esperado bajo el modelo.

### Capacidad predictiva del modelo

Otro aspecto a evaluar en un modelo de RLog es la *discriminación* (grado en que el modelo distingue entre individuos en los que ocurre el evento y los que no). Como medida de la discriminación se usa el área bajo la **curva ROC** (*receiver operating characteristic*), construida para la probabilidad predicha por el modelo. Quienes trabajen en diagnóstico estarán familiarizados con los conceptos de sensibilidad, especificidad y curvas ROC, pero de todas formas los explicaremos en detalle.
...
Se resume la capacidad predictiva de un modelo de RLog mediante el concepto de **sensibilidad**, es decir, la predicción de que *el suceso ha ocurrido cuando es cierto:*

$P(\hat{y}= 1|y=1)$

y mediante el concepto de **especificidad**, o predicción de que *el suceso no ha ocurrido cuando es cierto*:
...
Esta curva representa, para todos los pares posibles de individuos formados por un individuo en el que ocurrió el evento y otro en el que no, la proporción de los que el modelo predice una mayor probabilidad para el que tuvo el evento.
...
-   Comparación de modelos por LRT, AIC o ANOVA.
...
La regresión logística es uno de los modelos de la familia de modelos lineales generalizados (GLM por su nombre en inglés, *Generalized Linear Models*), utilizado para predecir la probabilidad de que ocurra un evento binario (infectado/no infectado, enfermo/no enfermo, sobreviviente/fallecido, etc.) en función de una o más variables independientes. Los GLM son una extensión de los modelos de regresión lineales que permiten utilizar distribuciones no normales de los errores (binomiales, Poisson, gamma, etc) y varianzas no constantes. Se caracterizan por tener una estructura de errores particular y una función de enlace entre la variable respuesta y la(s) variable(s) independiente(s).
...
El ajuste de un modelo de regresión logística puede hacerse a través de la función `glm()`, que sirve para modelar la mayor parte de los GLMs.
...
-   `family`: Hace referencia a la familia de distribuciones y la función de enlace elegida para el ajuste de este modelo. Las opciones disponibles son:

    -   `gaussian()`: se usa cuando la variable dependiente es continua y tiene distribución normal. Su función de enlace es la identidad (`link = "identity"`) y equivale al ajuste de un modelo de regresión lineal. También admite las funciones de enlace logaritmo (`link = "log"`) e inversa (`link = "inverse"`).

        ```{r}
        # Carga base ejemplo unidad 3
        datos <- read_csv2("state.txt")

        # Regresión lineal con lm()
        mod1 <- lm(Life.Exp ~ Murder + HS.Grad + Frost, data = datos)

        # Regresión lineal con glm()
        mod2 <- glm(Life.Exp ~ Murder + HS.Grad + Frost, data = datos,
                    family = gaussian(link = "identity"))

        # Comparo modelos
        compare_performance(mod1, mod2, metrics = "common")
        ```

    -   `binomial()`: usada cuando la variable dependiente es binomial. Tiene como función de enlace por defecto el logit (`link = "logit")`, que es el que usaremos en la regresión logística. También admite los enlaces probit (`link = "probit"`), cauchit (`link = "cauchit"`), logaritmo (`link = "log"`) y cloglog (`link = "cloglog"`).

    -   `poisson()`: usada para variables dependientes numéricas discretas. Tiene como función de enlace por defecto el logaritmo (`link = "log"`), permitiendo también los las funciones de enlace identidad (`link = "identity"`) y raíz cuadrada (`link = "sqrt"`).

    -   `Gamma()`: usada para variables dependientes numéricas continuas positivas (valores entre 0 y +$\infty$).
...
-   `data`: Indica el nombre de la base de datos (*dataframe* o *dataset*) que contiene las variables del modelo.

### Partes del modelo de regresión

Al igual que para la regresión lineal, la salida de resultados de la función `glm()` puede obtenerse a través de la función `summary()`. La sintaxis es:
...
El objeto de regresión está compuesto por 30 componentes que pueden accederse a través del nombre del modelo seguido del signo `$` y el nombre del componente. Entre los componentes más relevantes podemos señalar (usaremos `modelo` como generalización del nombre del objeto creado):

-   `modelo$coeficients`: vector de coeficientes del modelo. También se puede obtener a través de la función `coef(modelo)`

-   `modelo$residuals`: vector que contiene los residuos obtenidos en la última iteración.

-   `modelo$fitted.values`: vector con los valores medios ajustados, obtenidos según la transformación de los predictores lineales por la inversa de la función de enlace.

-   `modelo$family`: devuelve la familia utilizada en la construcción del modelo.

-   `modelo$deviance`: deviance del modelo ajustado o -2 veces el máximo de la log verosimilitud.

-   `modelo$aic`: criterio de información de Akaike (AIC)

-   `modelo$null.deviance`: deviance para el modelo nulo que solo contiene la constante.
